...
namespace InfinityCode.RunDesiredScene
{
    ...
    public static class ToolbarManager
    {
        ...
        private static MethodInfo ueeAddLeftMethod;
        private static MethodInfo ueeAddRightMethod;
        ...
        {
            if (ueeAddLeftMethod != null)
            {
                ueeAddLeftMethod.Invoke(null, new object[] {key, action, order});
            }
            else
            {
        ...
        {
            if (ueeAddRightMethod != null)
            {
                ueeAddRightMethod.Invoke(null, new object[] {key, action, order});
            }
            else
            {
        ...
        private static bool CheckUEE()
        {
            Assembly assembly = ReflectionHelper.GetAssembly("UltimateEditorEnhancer-Editor");
            if (assembly == null) return false;
            
            Type type = assembly.GetType("InfinityCode.UltimateEditorEnhancer.ToolbarManager");
            if (type == null) return false;
            
            ueeAddLeftMethod = type.GetMethod("AddLeftToolbar", ReflectionHelper.StaticLookup);
            ueeAddRightMethod = type.GetMethod("AddRightToolbar", ReflectionHelper.StaticLookup);

            if (ueeAddLeftMethod == null || ueeAddRightMethod == null) return false;
            
            for (int i = 0; i < leftToolbarCount; i++)
            {
                Item item = leftToolbarItems[i];
                if (item != null)
                {
                    ueeAddLeftMethod.Invoke(null, new object[] { item.key, item.action, item.order });
                }
            }

            for (int i = 0; i < rightToolbarCount; i++)
            {
                Item item = rightToolbarItems[i];
                if (item != null)
                {
                    ueeAddRightMethod.Invoke(null, new object[] { item.key, item.action, item.order });
                }
            }
