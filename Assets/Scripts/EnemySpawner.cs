    ...
    private void Update()
    {
        if (!isStoped)
        {
            if (timer >= Mathf.Max(minSpawnInterval, spawnInterval - (spawnCounter / spawnBoostThreshold) * spawnBoost))
            {
                ...
                var newEnemy = Instantiate(GetEnemyType(),
                    point.position + new Vector3(Random.Range(spawnRange, -spawnRange), 0, 0), Quaternion.identity);
                var speed = Random.Range(4, Mathf.CeilToInt(spawnCounter * 0.1f));
                if (speed < 2)
                    speed = 2;
                newEnemy.Initialize(experienceController, true, 
                    Random.Range(1, Mathf.CeilToInt(spawnCounter * 0.1f)),speed);
                spawnCounter++;
            }
