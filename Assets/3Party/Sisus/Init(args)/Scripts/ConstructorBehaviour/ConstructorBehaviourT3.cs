...
namespace Sisus.Init
{
	...
	public abstract class ConstructorBehaviour<TFirstArgument, TSecondArgument, TThirdArgument> : MonoBehaviour, IInitializable<TFirstArgument, TSecondArgument, TThirdArgument>, ISerializationCallbackReceiver
	{
		private static readonly ConcurrentDictionary<Type, (string, string, string)> initPropertyNamesCache = new ConcurrentDictionary<Type, (string, string, string)>();
		...
		{
			dependenciesReceived = InitArgs.TryGet(Context.Constructor, this, out firstArgument, out secondArgument, out thirdArgument);
        ...
        {
			if(dependenciesReceived && GetIsAnyInitPropertySerialized() && InitArgs.TryGet(Context.OnAfterDeserialize, this, out TFirstArgument firstArgument, out TSecondArgument secondArgument, out TThirdArgument thirdArgument))
            {
