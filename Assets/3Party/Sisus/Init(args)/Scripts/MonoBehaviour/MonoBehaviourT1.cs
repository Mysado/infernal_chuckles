...
[assembly: InternalsVisibleTo("Initializers")]

namespace Sisus.Init
{
	/// <summary>
	/// A base class for <see cref="MonoBehaviour">MonoBehaviours</see> that can be
	/// <see cref="ObjectExtensions.Instantiate{TComponent, TArgument}">instantiated</see>
	...
	public abstract class MonoBehaviour<TArgument> : MonoBehaviour, IInitializable<TArgument>, IInitializable
	{
		#if DEBUG
		/// <summary>
		...
		private bool Init(Context context)
		{
			...
			#if DEBUG
			initializing = true;
			if(context != Context.EditMode) { ValidateArgument(argument); }
			#endif
