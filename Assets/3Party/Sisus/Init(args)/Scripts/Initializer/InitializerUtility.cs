...
namespace Sisus.Init.Internal
{
	internal static class InitializerUtility
	{
		...
		private static readonly List<IInitializer> initializers = new List<IInitializer>();
		...
		internal static void Validate_NotThreadSafe<TInitializer, TArgument>(TInitializer initializer, [AllowNull] GameObject gameObject, Any<TArgument> argument) where TInitializer : Object, IInitializerEditorOnly
		{
			...
			if(!TryBegin_Validate_NotThreadSafe(initializer, gameObject))
			{
				return;
			}
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private static bool TryBegin_Validate_NotThreadSafe<TInitializer>(TInitializer initializer, GameObject gameObject) where TInitializer : Object, IInitializerEditorOnly
		{
			if(initializer == null)
			{
				return false;
			...
			(TInitializer initializer) where TInitializer : Object, IInitializerEditorOnly<TClient, TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument>
		{
			...
			if(AutoInit_Argument(out TSecondArgument secondArgument, 1, true, initializer)) initializer.SecondArgument = secondArgument;
			if(AutoInit_Argument(out TThirdArgument thirdArgument, 2, true, initializer)) initializer.ThirdArgument = thirdArgument;
			if(AutoInit_Argument(out TFourthArgument fourthArgument, 3, true, initializer)) initializer.ThirdArgument = thirdArgument;
		}
		...
		private static void Reset_Initializer<TInitializer, TTarget>([DisallowNull] TInitializer initializer, GameObject gameObject) where TInitializer : Object, IInitializerEditorOnly<TTarget>
        {
			...
			if(TryFindComponentWithoutInitializer(gameObject, typeof(TTarget), out Component setTarget))
			{
	            initializer.Target = setTarget;
			}
        }

		private static bool TryFindComponentWithoutInitializer(GameObject gameObject, Type clientType, out Component result)
		{
			gameObject.GetComponents(initializers);
			...
			foreach(Type componentType in componentTypes)
			{
				...
				{
					if(!initializers.Any(i => i.Target == component))
					{
						result = component;
						return true;
					}
		...
		{
			var setHideFlags = !(initializer.Target is Component clientComponent) || !clientComponent || clientComponent.gameObject != gameObject ? HideFlags.None : HideFlags.HideInInspector;
