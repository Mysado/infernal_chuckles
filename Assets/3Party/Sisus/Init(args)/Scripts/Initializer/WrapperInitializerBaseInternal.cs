...
namespace Sisus.Init.Internal
{
		...
		where TWrapper : Wrapper<TWrapped>
	{
		...
		private protected NullArgumentGuard nullArgumentGuard = NullArgumentGuard.EditModeWarning | NullArgumentGuard.RuntimeException;
		...
		/// <inheritdoc/>
		ValueTask<object> IInitializer.InitTargetAsync() => InitTargetAsync().AsObjectValueTask();

		/// <inheritdoc/>
		public async ValueTask<TWrapped> InitTargetAsync()
		{
			if(this == null)
			{
				return target;
		...
		private protected virtual ValueTask<TWrapper> InitTargetAsync(TWrapper target) => new ValueTask<TWrapper>(InitTarget(target));
