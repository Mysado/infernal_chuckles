...
{
	/// <summary>
	/// A base class for a component that can specify the six arguments used to initialize a component that implements
			...
			where TClient : MonoBehaviour, IInitializable<TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument, TFifthArgument, TSixthArgument>
	{
		...
		private protected sealed override async ValueTask<TClient> InitTargetAsync(TClient target)
		{
			...
			#endif
            {
				gameObject.AddComponent(out target, firstArgument, secondArgument, thirdArgument, fourthArgument, fifthArgument, sixthArgument);
                return target;
            }
