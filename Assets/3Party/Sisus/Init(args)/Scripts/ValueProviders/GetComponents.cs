...
namespace Sisus.Init.ValueProviders
{
	...
	#endif
	{
		private const string MENU_NAME = "Get Components";
		...
		#if UNITY_EDITOR
		NullGuardResult INullGuardByType.EvaluateNullGuard<TValue>(Component client) // => client != null && Find.typesToFindableTypes.ContainsKey(typeof(TValue)) ? NullGuardResult.Passed : NullGuardResult.ValueProviderValueNullInEditMode;
		{
			if(client == null)
			{
