...
{
	/// <summary>
	/// Returns all objects of the given type attached to the <see cref="GameObject"/> and all its children.
	/// <para>
	/// Can be used to retrieve an Init argument at runtime.
	/// </para>
	/// </summary>
	#if !INIT_ARGS_DISABLE_VALUE_PROVIDER_MENU_ITEMS
	[ValueProviderMenu(MENU_NAME, WhereAll = Is.Collection | Is.SceneObject, Not = typeof(GameObject), Order = 10, Tooltip = "Collection will be created at runtime from the components attached to this game object and all its children.")]
	#endif
	#if DEV_MODE
	[CreateAssetMenu(fileName = MENU_NAME, menuName = CREATE_ASSET_MENU_GROUP + MENU_NAME)]
	#endif
	internal sealed class GetComponentsInChildren : ScriptableObject, IValueByTypeProvider
	#if UNITY_EDITOR
	, INullGuardByType
	#endif
	{
		private const string MENU_NAME = "Get Components In Children";

		/// <summary>
		/// Gets all objects attached to the <paramref name="client"/> and all its children that match the element type of the <typeparamref name="TValue"/> collection.
		/// </summary>
		...
		public bool TryGetFor<TValue>(Component client, out TValue value)
		{
			...
			var found = Find.AllInChildren(client.gameObject, GetCollectionElementType(typeof(TValue)), client.gameObject.activeInHierarchy);
