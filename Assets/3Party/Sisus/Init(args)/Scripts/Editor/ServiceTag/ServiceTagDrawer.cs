...
namespace Sisus.Init.EditorOnly.Internal
{
	...
	internal static class ServiceTagDrawer
	{
		...
		private static float OnBeforeHeaderGUI(Component component, Rect headerRect, bool HeaderIsSelected, bool supportsRichText)
		{
			...
			if(ServiceTagUtility.openSelectTagsMenuFor == serviceOrServiceProvider)
			{
				ServiceTagUtility.openSelectTagsMenuFor = null;
				ServiceTagUtility.OpenSelectTagsMenu(serviceOrServiceProvider, tagRect);
				return 0f;
			}
			...
			if(GUI.Button(tagRect, GUIContent.none, EditorStyles.label))
			{
				...
				{
					case 0:
						ServiceTagUtility.OpenToClientsMenu(serviceOrServiceProvider, tagRect);
						break;
					case 1:
						ServiceTagUtility.OpenContextMenuForService(serviceOrServiceProvider, tagRect);
						break;
					case 2:
						ServiceTagUtility.PingServiceDefiningObject(serviceOrServiceProvider);
						break;
				}
			}
		...
		private static Rect GetTagRect(Component service, Rect headerRect, GUIContent label, GUIStyle style)
        {
			...
			{
				var refTagRect = ServiceTagUtility.GetTagRect(service, headerRect, refLabel, style);
