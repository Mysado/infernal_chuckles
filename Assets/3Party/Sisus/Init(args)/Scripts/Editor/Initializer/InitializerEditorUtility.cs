...
namespace Sisus.Init.EditorOnly.Internal
{
	internal static class InitializerEditorUtility
	{
		...
		{
			if(InitializerUtility.TryGetInitializer(initializable, out IInitializer initializer))
			{
				foreach(Type interfaceType in initializer.GetType().GetInterfaces())
				{
					...
					if(genericTypeDefinition == typeof(IInitializer<,>)
						|| genericTypeDefinition == typeof(IInitializer<,,>)
						|| genericTypeDefinition == typeof(IInitializer<,,,>)
						|| genericTypeDefinition == typeof(IInitializer<,,,,>)
						|| genericTypeDefinition == typeof(IInitializer<,,,,,>)
						|| genericTypeDefinition == typeof(IInitializer<,,,,,,>))
		...
		public static void GenerateAndAttachInitializer(Object[] clients, MonoScript monoScript)
		{
			...
			EditorPrefs.SetString(InitializerGUI.SetInitializerTargetOnScriptsReloadedKey, initializerGuid + ":" + string.Join(";", clients.Select(t => t.GetInstanceID())));
		...
		private static bool IsInitializerFor(Type initializerType, Type clientType)
		{
			...
			foreach(Type interfaceType in initializerType.GetInterfaces())
			{
				...
				if(genericTypeDefinition == typeof(IInitializer<,>)
					...
					|| genericTypeDefinition == typeof(IInitializer<,,,,,,>))
				{
					return interfaceType.GetGenericArguments()[0].IsAssignableFrom(clientType);
				}
			}
		...
		[return: MaybeNull]
		internal static bool TryGetInitParameterAttributesFromMetadata(string initializerPropertyName, Type parameterType, Type metadataClass, [NotNullWhen(true)][MaybeNullWhen(false)] out Attribute[] results)
		{
		...
		public static AnyGUI[] GetPropertyDrawerData(SerializedObject serializedObject, Type clientType, Type[] argumentTypes, InitializerGUI drawer)
			=> GetPropertyDrawerData(serializedObject, clientType, argumentTypes, GetAnyFieldSerializedProperties(serializedObject, argumentTypes), drawer);
		...
		private static SerializedProperty[] GetAnyFieldSerializedProperties(SerializedObject serializedObject, Type[] argumentTypes)
		{
			...
			var usualPropertyNames = propertyNamesByArgumentCount[count];
			for(int i = 0; i < count; i++)
			{
		...
		private static AnyGUI[] GetPropertyDrawerData(SerializedObject serializedObject, Type clientType, Type[] argumentTypes, SerializedProperty[] serializedProperties, InitializerGUI initializerDrawer)
		{
			...
			var results = new AnyGUI[argumentCount];
            ...
            if(members.Length == argumentCount + 1)
			{
				for(int i = 0, count = serializedProperties.Length; i < count; i++)
				{
					...
					if(serializedProperty is null)
					{
						...
						var newResults = new AnyGUI[results.Length - 1];
						if(i < count - 1)
						{
