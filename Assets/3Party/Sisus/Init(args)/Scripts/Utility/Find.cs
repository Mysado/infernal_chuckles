...
namespace Sisus.Init
{
    ...
    public static class Find
    {
        ...
        public static bool In([DisallowNull] GameObject gameObject, [DisallowNull] Type type, [NotNullWhen(true), MaybeNullWhen(false)] out object result)
        {
            ...
            for(int i = findableTypes.Length - 1; i >= 0; i--)
            {
                if(InGameObjectByExactType(gameObject, findableTypes[i], out Component component))
                {
                    result = As(type, component);
                    return result is not null;
                }
        ...
        public static T Asset<T>()
        {
            ...
            if(typeof(T).IsAbstract && typeof(Object).IsAssignableFrom(typeof(T)))
			{
			    ...
			    foreach(var derivedType in TypeUtility.GetDerivedTypes(typeof(T), typeAssembly, null))
			    {
        ...
        private static void SetupInterfaceType([DisallowNull] List<Type> reusableList, [DisallowNull] Type unityObjectType, [DisallowNull] Type interfaceType)
        {
        ...
        private static bool InGameObjectByExactType<T>([DisallowNull] GameObject gameObject, [DisallowNull] Type findableType, [NotNullWhen(true), MaybeNullWhen(false)] out T result)
        {
            if(!typeof(Component).IsAssignableFrom(findableType))
            {
                if(typeof(T) == typeof(GameObject))
        ...
        #if !UNITY_2020_1_OR_NEWER
        private static void ComponentsByExactTypeInLoadedScenesIncludingInactive<T>([DisallowNull] Type findableType, [DisallowNull] List<T> results)
        {
            #if DEBUG
            if(findableType.IsGenericTypeDefinition)
            {
        ...
        #if !UNITY_2020_1_OR_NEWER
        private static Component ComponentByExactTypeInLoadedScenesIncludingInactive(Type type)
        {
            var list = Cached<GameObject>.list;
