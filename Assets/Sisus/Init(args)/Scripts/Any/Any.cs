...
namespace Sisus.Init
{
		...
		#endif
	{
			...
			if(reference is not null && reference.GetHashCode() != 0)
			{
				switch(reference)
				{
					...
					case IValueProvider<T> valueProviderT:
						return CacheValueProviderResult(valueProviderT.Value);
					...
					case IValueProviderAsync<T> valueProvider:
						var task = valueProvider.GetForAsync(client);
						return task.IsCompletedSuccessfully ? CacheValueProviderResult(task.Result) : default;
					case IValueByTypeProviderAsync valueProvider:
						var taskByType = valueProvider.GetForAsync<T>(client);
						return taskByType.IsCompletedSuccessfully ? CacheValueProviderResult(taskByType.Result) : default;
				}
			}
		...
		/// otherwise, the default value of <see cref="T"/>.
		/// </returns>
		public async ValueTask<T> GetValueAsync(Component client = null, Context context = Context.MainThread)
		{
			// Prefer cached value over a value provider - this can help avoid issues
			// such as addressable assets being loaded more than once by the same client
			// and causing the internal counter to get incremented too many times.
			...
			return reference switch
			{
				...
				IValueProvider<T> valueProvider => CacheValueProviderResult(valueProvider.Value),
			...
			if(reference is not null && reference.GetHashCode() != 0)
			{
				switch(reference)
				{
					...
					case IValueProvider<T> valueProvider:
						return valueProvider.Value is not null;
					case IValueByTypeProvider valueProvider:
			...
			if(reference is not null && reference.GetHashCode() != 0)
			{
				switch(reference)
				{
					...
					case IValueProviderAsync<T> valueProvider:
						...
						{
							result = CacheValueProviderResult(task.Result);
							return true;
						}
					...
					case IValueByTypeProviderAsync valueProvider:
						var taskByType = valueProvider.GetForAsync<T>(client);
						if(taskByType.IsCompletedSuccessfully)
						{
							result = CacheValueProviderResult(taskByType.Result);
							return true;
						}
		...
		internal NullGuardResult EvaluateNullGuard(Component client = null, Context context = Context.MainThread)
		{
			if(reference is not null && reference.GetHashCode() != 0)
			{
				switch(reference)
				{
					...
					case IValueProviderAsync<T> valueProvider:
						var task = valueProvider.GetForAsync(client);
						return task.IsFaulted ? NullGuardResult.ExceptionOccurred : NullGuardResult.Passed;
					case IValueByTypeProviderAsync valueProvider:
						var taskByType = valueProvider.GetForAsync<T>(client);
						return taskByType.IsFaulted ? NullGuardResult.ExceptionOccurred : NullGuardResult.Passed;
				}
			}
