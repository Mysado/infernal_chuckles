...
namespace Sisus.Init.EditorOnly.Internal
{
	internal sealed class InitializerGUI : IDisposable
	{
		...
		private GUIContent nullGuardPassedWithValueProviderValueMissing;
		...
		private readonly bool targetCanForSureSelfInitialize;
		private readonly bool targetCanMaybeSelfInitialize;
		...
		/// <param name="initializerEditor"></param>
		...
		public InitializerGUI(Type clientType, Object[] clients, Type[] initParameterTypes, Editor initializerEditor = null, GameObject[] gameObjects = null)
		{
			#if DEV_MODE
			...
			hasServiceParameters = initServiceParameters.Length > 0 && initServiceParameters.Any(isService => isService);
			allParametersAreServices = initServiceParameters.Length > 0 && initServiceParameters.All(isService => isService);
			
			targetCanMaybeSelfInitialize = allParametersAreServices;
			targetCanForSureSelfInitialize = allParametersAreServices && TypeUtility.DerivesFromGenericBaseType(clientType);
		...
		private void Setup()
		{
			...
			nullGuardPassedWithValueProviderValueMissing = EditorGUIUtility.IconContent("DebuggerAttached");
		...
		public static void OnAssemblyCompilationStarted(ref InitializerGUI drawer, string compilingAssemblyName)
		{
		...
		public void OnInspectorGUI()
		{
			...
			bool initializerUnfolded = !drawInitHeader ||
			...
			if(drawAddInitializerButton)
			{
				...
				if(targetCanMaybeSelfInitialize)
				{
					...
					nullGuardPassedIcon.tooltip
						...
						? "All arguments are services.\n\nAdding an initializer is not necessary."
						: "All arguments are services.\n\nThe client can use InitArgs.TryGet to receive the arguments during initialization, in which case adding an initializer is not necessary.";
			...
			else
			{
				...
				if(drawNullGuard)
				{
					...
					//{
					//	GUI.color = Color.yellow; // tint the blue icon yellow, so it becomes green
					//}
		...
		private string GetInitArgumentsTooltip([DisallowNull] Type clientType, [DisallowNull] Type[] initParameterTypes, [DisallowNull] bool[] initServiceParameters)
		{
