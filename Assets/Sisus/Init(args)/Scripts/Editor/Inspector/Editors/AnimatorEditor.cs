...
{
	[CustomEditor(typeof(Animator), editorForChildClasses: true, isFallback = false), CanEditMultipleObjects]
    public sealed class AnimatorEditor : Editor
	{
		private static readonly Type internalEditorType = typeof(Editor).Assembly.GetType("UnityEditor.AnimatorInspector");

		private static GUIContent iconToolbarPlus;
		private static GUIStyle preButton;
		private static GUIStyle footerBackground;

		private Editor animatorEditor;
		private readonly List<InitializerGUI> initializerDrawers = new List<InitializerGUI>();
			...
			foreach(var behaviourAndInitializableType in GetInitializableBehavioursInAnimator(FirstAnimator))
			{
				...
				foreach(var initializerType in InitializerEditorUtility.GetInitializerTypes(behaviourType))
				{
					...
					for(int i = 0, initializerCount = gameObjects[0].GetComponents(initializerType).Length; i < initializerCount; i++)
					{
						...
						var drawer = new InitializerGUI(behaviourType, new Object[] { behaviour }, initParameterTypes, null, gameObjects);
		...
		private void OnAddButtonPressed(Rect addRect)
		{
			...
			foreach(var behaviourAndInitializableType in GetInitializableBehavioursInAnimator(FirstAnimator))
			{
				...
				{
					menu.AddItem(new GUIContent("Generate Initializer For " + behaviourType.Name), false, () => InitializerEditorUtility.GenerateAndAttachInitializer(targets, Find.Script(behaviourType)));
		...
		private IEnumerable<Type> GetAllAddableInitializerTypes(Animator animator)
		{
			...
			{
				var stateMachineBehaviour = behaviourAndInitializableType.behaviour;
				foreach(var initializerType in InitializerEditorUtility.GetInitializerTypes(stateMachineBehaviour.GetType()))
				{
