...
namespace Sisus.Init.EditorOnly
{
	...
	public class InitializableEditor : Editor
    {
		...
		[NonSerialized]
		private InitializerGUI initializerDrawer;
		...
		{
			if(!EditorPrefs.HasKey(Internal.InitializerGUI.SetInitializerTargetOnScriptsReloadedKey))
			{
				return;
			}

			var value = EditorPrefs.GetString(Internal.InitializerGUI.SetInitializerTargetOnScriptsReloadedKey);
			EditorPrefs.DeleteKey(Internal.InitializerGUI.SetInitializerTargetOnScriptsReloadedKey);
		...
		private void OnAssemblyCompilationFinished(string assemblyPath, CompilerMessage[] compilerMessages)
		{
			...
			Internal.InitializerGUI.OnAssemblyCompilationStarted(ref initializerDrawer, assemblyName);
		}
		...
		protected virtual void Setup()
		{
			...
			initializerDrawer = new InitializerGUI(targetType, targets, initParameterTypes);
