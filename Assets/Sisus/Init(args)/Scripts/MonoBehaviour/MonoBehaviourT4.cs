...
namespace Sisus.Init
{
	...
	/// <see cref="ObjectExtensions.Instantiate{TComponent, TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument}">instantiated</see>
	...
	public abstract class MonoBehaviour<TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument> : MonoBehaviour, IInitializable<TFirstArgument, TSecondArgument, TThirdArgument, TFourthArgument>, IInitializable
	{
		#if DEBUG
		/// <summary>
		...
		private bool Init(Context context)
		{
			...
			#if DEBUG
			initializing = true;
			if(context != Context.EditMode) { ValidateArguments(firstArgument, secondArgument, thirdArgument, fourthArgument); }
			#endif
