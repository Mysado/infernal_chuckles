...
{
	/// <summary>
	/// Loads an addressable <see cref="Texture"/> asset with the given key.
	/// <para>
		...
		#endif
	{
		...
		protected override void Init(AssetReferenceTexture addressableAsset) => this.addressableAsset = addressableAsset;
		...
		/// </returns>
		public async ValueTask<Texture> GetForAsync([AllowNull] Component client)
		{
			#if UNITY_EDITOR
			...
			{
				return await addressableAsset.LoadAssetAsync<Texture>().Task;
			}

			return addressableAsset.Asset as Texture;
		}

		async ValueTask<object> IValueProviderAsync.GetForAsync([AllowNull] Component client) => await GetForAsync(client);
		...
		public bool Equals(LoadAddressableTexture other) => other is not null && addressableAsset == other.addressableAsset;
		public override bool Equals(object other) => other is LoadAddressableTexture loadAddressable && Equals(loadAddressable);
		public override int GetHashCode() => addressableAsset?.AssetGUID?.GetHashCode() ?? 0;

		public void Release([AllowNull] Component client, Texture value)
		{
			#if UNITY_EDITOR
			if(!Application.isPlaying)
			{
