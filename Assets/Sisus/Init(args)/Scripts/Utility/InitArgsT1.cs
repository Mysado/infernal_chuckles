...
namespace Sisus.Init
{
	...
	/// <see cref="IArgs{TArgument}"/> interface such as <see cref="MonoBehaviour{TArgument}"/>
	/// and <see cref="ScriptableObject{TArgument}"/>.
	/// </summary>
	/// <typeparam name="TArgument"> Type of the argument. </typeparam>
	public static class InitArgs<TArgument>
	{
		/// <summary>
		/// Calls the <see cref="IInitializable{TArgument}.Init">Init</see> function on the <paramref name="client"/>
		/// with the argument previously stored using the <see cref="InitArgs.Set{TArgument}">Set</see> function.
		/// <para>
		...
		/// </para>
		/// <para>
		/// The <paramref name="client"/> must implement the <see cref="IInitializable{TArgument}"/> interface in order for it to be used with this function.
		/// </para>
		/// </summary>
		...
		public static bool TryGet<TClient>(Context context, [DisallowNull] TClient client) where TClient : IInitializable<TArgument>
		{
